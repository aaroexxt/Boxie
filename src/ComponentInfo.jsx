...
export default function ComponentInfo(props) {
  ...
  let component = props.fetched.component;
  let code = (
    "B" +
    props.fetched.boxNum +
    "-S" +
    props.fetched.sectionNum +
    "-" +
    props.fetched.sectionRow +
    "-" +
    props.fetched.sectionCol
  ).trim();
  ...
  switch (component.type) {
    ...
    case cDefs.types.CAPACITOR:
      content.push(
        <p>
          Value: {component.additional.value + component.additional.valueUnit}
        </p>
      );
      break;
    case cDefs.types.IC:
  ...
  return <Paper className={classes.componentInfo}>{content}</Paper>;
}
