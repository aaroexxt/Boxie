...
import { getFilledSpaceInBox } from "./utils/lookup.js";
  ...
  if (props.show) {
    return (
      <Button
        variant="outlined"
        onClick={() => {
          props.toggleQuantityOverlay();
        }}
      >
        Toggle Quantity Overlay
      </Button>
    );
  } else {
    return null;
...
class StorageBox extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      boxEmpty: getFilledSpaceInBox(this.props.box) == 0,
    };
  ...
  handleLayoutChange(layout, idx) {
    ...
    if (illegalSection) {
      ...
      this.props.handleBoxChange(
        this.layoutsToBox(layout, idx, this.props.box)
      ); //Actually fire box save event
    }
  }
  ...
  handleComponentHover(uuid) {
    if (uuid == null || typeof uuid == "undefined") {
      ...
      let componentAndPosition = this.props.getComponentAndPosition(uuid);
      this.state.componentPopupRef = componentAndPosition;
  ...
  renderBox(box) {
    ...
    if (this.state.boxEmpty) {
      return <h3>Box Empty</h3>;
    } else {
      let sections = box.sections;
      let content = [];
      sections.map((section, idx) => {
            ...
            if (
              ...
              switch (component.type) {
                ...
                case cDefs.types.CAPACITOR:
                  info =
                    component.additional.value + component.additional.valueUnit;
                  break;
                case cDefs.types.IC:
                ...
                case cDefs.types.CRYSTAL:
                  info =
                    component.additional.frequency +
                    component.additional.frequencyUnit;
                  break;
              ...
              if (info.length > 6) info = info.substring(0, 6); //ensure it's not too long
              ...
              sContent.push(
                <div key={uuid} className={classes.paper}>
                  <Paper
                    onMouseEnter={() => {
                      this.handleComponentHover(uuid);
                    }}
                    onMouseLeave={() => {
                      this.handleComponentHover(null);
                    }}
        ...
        if (sectionEmpty) {
          content.push(
            <div key={idx}>
              ...
              <h4 style={{ paddingLeft: "20px" }}>Section Empty</h4>
            </div>
          );
        } else {
          content.push(
  ...
  handleRestoreButton() {
    this.setState({
      illegalDimension: false,
      layouts: this.boxToLayouts(this.props.box),
    });
  }
  ...
  render() {
    ...
    return (
      <div>
        ...
        <QuantityButton
          show={!this.state.boxEmpty}
          toggleQuantityOverlay={() => {
            this.toggleQuantityOverlay();
          }}
        />
        ...
        >
          <Paper className={classes.dimWarn}>
            <div className={classes.flexCenter}>
              <WarningIcon /> &nbsp; Warning: Box has an invalid dimension.
        ...
        <Popover
          ...
          className={classes.popover}
        >
          <ComponentInfo fetched={this.state.componentPopupRef} />
        </Popover>
      </div>
    );
  }
