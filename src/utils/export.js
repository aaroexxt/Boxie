...
import { componentDefinitions as cDefs } from "./componentDefinitions.js";
 ...
 * @param  {Number} uIn Inches to convert
 * @return {Number}     Converted pixels
 */
const inToPx = (uIn) => {
  return uIn * ppi;
};
...
/**
 * Sets up a basic canvas for drawing
 * @return {Array} Created canvas and its 2d context
 */
const initBasicCanvas = () => {
...
/**
 * Gets height and width of the canvas
 * @return {Object} Height and width of the canvas
 */
...
/**
 * Exports a set of base-64 encoded images that are rendered sheets of Labels
 * @param  {Array} list         List of components to be rendered
 * @param  {Array} boxes        List of boxes to draw components from
 * @param  {Function} dataCallback Function to call for every valid label exported; takes base64-encoded image as argument
 * @return {Promise}              Promise resolves or rejects based on whether function exported data correctly
 */
export const exportComponentLabels = (list, boxes, dataCallback) => {
  return new Promise((resolve, reject) => {
    ...
    for (let i = 0; i < list.length; i++) {
      ...
      switch (component.type) {
        ...
        case cDefs.types.CRYSTAL:
          info =
            component.additional.frequency + component.additional.frequencyUnit;
          break;
    ...
    if (parsedComponents.length == 0) {
      return reject(
        "Couldn't find any components that are assigned; make sure components have all been assigned before exporting"
      );
    }

    //Now recursively call sheet until we've reached the end of the list
    let sheet = (idx, num) => {
      //idx of component, sheet number
      ...
      while (y < canvasHeightPx && idx < hardStopIdx) {
        //Row
        let rowComponentMaxHeight = 0;
        while (x < canvasWidthPx && y < canvasHeightPx && idx < hardStopIdx) {
          //Column
          if (idx > parsedComponents.length) {
            dataCallback(canvas.toDataURL()); //save the current canvas
            console.log(
              "Successfully exported " + num + " canvas(ses) of labels"
            );
          ...
          if (!component) {
            //if the component isn't found, skip
            idx++;
            console.warn(
              "Invalid component found (parsedComponents object missing), has it not been assigned to a box yet?"
            );
            continue;
          }
          ...
          for (let i = 0; i < dimKeys.length; i++) {
            if (
              dimKeys[i].toLowerCase() == component.sectionType.toLowerCase()
            ) {
              //dim key matches i.e. "small"
              foundDim = true;
              cDim = componentLabelDims[dimKeys[i]];
            }
          }
          if (!foundDim) {
            //if it doesn't match an dimension, skip
            idx++;
            console.warn(
              "Component found that does not match any existing dimension"
            );
            continue;
          }
          if (cDim[0] > rowComponentMaxHeight) {
            //make sure we set the max height
            rowComponentMaxHeight = cDim[0];
          }

          if (cDim[0] > canvasHeightPx || cDim[1] > canvasWidthPx) {
          ...
          if (x + cDim[1] > canvasWidthPx) {
            //if this will go further than it should, then end row early
            x += cDim[1]; //will trigger new row next cycle
            continue;
          }
          if (y + cDim[0] > canvasHeightPx) {
            //if this will go further than it should, then end sheet early
          ...
          while (
            ctx.measureText(value).width < cDim[1] - 15 &&
            fontSize <= maxSize
          ) {
          ...
          ctx.fillText(
            value,
            (cDim[1] - valueWidth) / 2 + x,
            y + inToPx(0.025) + (maxSize - fontSize) / 2
          );
          ...
          let code = (
            ...
            "-" +
            "S" +
            component.sectionNum +
            "-" +
            component.sectionRow +
            "-" +
            component.sectionCol
          ).trim();
          ctx.font = "25px Helvetica";
          const codeWidth = ctx.measureText(code).width;
          ctx.fillText(
            code,
            (cDim[1] - codeWidth) / 2 + x,
            y + cDim[0] - inToPx(0.125)
          );
      ...
      if (idx >= hardStopIdx) {
        return reject(
          "Uhh something has likely gone wrong with recursion (looped " +
            hardStopIdx +
            " times), remove this check if you want more than this many components"
        );
      } else {
        dataCallback(canvas.toDataURL());
...
/**
 * Exports a set of base64-encoded images that are a set of box title box sheets
 * @param  {Array} boxes        Array of boxes to render
 * @param  {Array} dataCallback Function to process each base64-encoded image
 * @return {Promise}              Resolves or rejects based on whether images were exported successfull
 */
export const exportBoxLabels = (boxes, dataCallback) => {
  return new Promise((resolve, reject) => {
    let boxPrintInfo = [];
    for (let i = 0; i < boxes.length; i++) {
      //Generate box labels
      boxPrintInfo.push([boxes[i].title, boxes[i].description, i + 1]);
    }

    let boxesPerSheet = Math.floor(
      canvasHeightPx / (heightBoxLabel + inToPx(0.1))
    );
    ...
    console.log(
      "Successfully exported " + boxSheets + " canvas(ses) of box labels"
    );
    return resolve();
  });
};
